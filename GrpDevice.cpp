...
bool CGraphicDevice::Reset()
{
	...
	if (FAILED(hr = ms_lpd3dDevice->TestCooperativeLevel()))
	{
		...
		if (hr == D3DERR_DEVICENOTRESET)
		{
			...
			if (m_pStateManager)
			{
				SAFE_DELETE(m_pStateManager); // @fix10 Directx9 Sonrasý zemin bozulmasý fix (1)
			...
			//m_pStateManager = new CStateManager(ms_lpd3dDevice); // @fix10 Directx9 Sonrasý zemin bozulmasý fix (2)
