...
void CScreen::RenderBar3d(float sx, float sy, float sz, float ex, float ey, float ez)
{
	...
	assert(ms_lpd3dDevice != NULL);
	STATEMANAGER.SetRenderState(D3DRS_MULTISAMPLEANTIALIAS, FALSE); // // @fix42
...
//	// 2003. 9. 9 µ¿Çö Ãß°¡
//	// ÁöÇü pickingÀ» À§ÇÑ »¹Áþ... ¤Ñ¤Ñ; À§¿¡ °Í°ú ÅëÇÕ ÇÊ¿ä...
...
BOOL CScreen::RestoreDevice()
{
	...
	IDirect3D9Ex& rkD3D = *reinterpret_cast<IDirect3D9Ex*>(ms_lpd3d);
	IDirect3DDevice9Ex& rkD3DDev = *reinterpret_cast<IDirect3DDevice9Ex*>(ms_lpd3dDevice);
	D3DPRESENT_PARAMETERS& rkD3DPP = ms_d3dPresentParameter;
	D3D_CDisplayModeAutoDetector& rkD3DDetector = ms_kD3DDetector;

	// DirectX 9Ex'te cihaz kaybý farklý ele alýndýðý için TestCooperativeLevel'ý kullanmaya gerek yok
	// Cihaz kaybý durumlarýný kendiliðinden yönettiðinden yalnýzca durumu kontrol ediyoruz.
	D3D_CAdapterInfo* pkD3DAdapterInfo = rkD3DDetector.GetD3DAdapterInfop(ms_iD3DAdapterInfo);

	if (!pkD3DAdapterInfo)
		return FALSE;

	// Masaüstü ekran modunu güncelleyelim
	D3DDISPLAYMODEEX rkD3DDMDesktopEx = {};
	rkD3DDMDesktopEx.Size = sizeof(D3DDISPLAYMODEEX);

	// DirectX 9Ex'in GetAdapterDisplayModeEx kullanýmý
	if (FAILED(rkD3D.GetAdapterDisplayModeEx(iD3DAdapterInfo, &rkD3DDMDesktopEx, nullptr)))
		return FALSE;

	rkD3DPP.BackBufferFormat = rkD3DDMDesktopEx.Format;

	// DirectX 9Ex cihazý için ResetEx kullanýmý
	HRESULT hrReset = rkD3DDev.ResetEx(&rkD3DPP, nullptr);
